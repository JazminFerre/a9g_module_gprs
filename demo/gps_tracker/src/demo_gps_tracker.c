...
void gps_testTask(void *pData)
{
    ...
    while(1)
    {
        if(isGpsOn)
        {
            ...
            //snprintf(buffer,sizeof(buffer), "/module/save-location/%f/%f/%f",latitude,latitude,longitude);
            //show in tracer
            Trace(1,buffer);
            //send to UART1
           ...
           //                                         buffer,time(NULL),latitude,longitude,isFixed*1.0,0.0,gpsInfo->gga.altitude,0.0,percent*1.0);
            snprintf(buffer15,sizeof(buffer2), "http://app-argus-server.herokuapp.com/module/save-location/%f/%f/%f",longitude,longitude,longitude);
            if(Http_Post(SERVER_IP,SERVER_PORT,buffer15,NULL,0,buffer,sizeof(buffer)) <0 )
                Trace(1,"send location to server fail");
            else
            {
                Trace(1,"send location to server success");
                Trace(1,"response:%s",buffer15);
            }
        }
