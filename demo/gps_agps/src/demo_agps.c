...
int Http_Post(const char* domain, int port,const char* path,uint8_t* body, uint16_t bodyLen, char* retBuffer, int bufferLen)
{
    ...
    snprintf(temp,2048,"GET %s HTTP/1.0\r\n\r\n",
                            path,bodyLen);
    char* pData = temp;
    ...
    //memcpy(&sockaddr.sin_addr.s_addr,servInetAddr->h_addr,servInetAddr->h_length);
...
void gps_testTask(void *pData)
{
    ...
    while(1)
    {
        if(isGpsOn)
        {
            ...
            snprintf(requestPath,sizeof(buffer2),"/ping");
            Trace(1,"serverpath = %s", requestPath);
